...
## Weka–Python Integration

The proposed framework uses Weka for wrapper-based feature selection and classifier evaluation, while Python is used for the downstream biological analysis.

To ensure consistency between the two environments:

- The same 80/20 train-test split was maintained throughout the workflow.
- Wrapper-based feature selection was performed in Weka.
- The final selected gene subsets generated by Weka were exported without modification.
- These selected gene subsets were directly imported into Python for weighted F-measure ranking, pathway-enrichment analysis, and gene-pathway network construction.
- No additional feature selection or classifier retraining was performed in Python.
- Therefore, all biological analyses were conducted using exactly the same selected gene subsets obtained from Weka.
